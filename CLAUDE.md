...
- **Dev server (30)**: 192.168.2.30 — git repo lives here, all editing happens here
- **Pi (83)**: 192.168.2.83 — production, runs the app for users
...
- Database: SQLite at instance/NRWCup2025.db
- `app_main.py` uses auto-detect db_path (portable across dev/Pi)
...
### Completed
- Analyzed Messwertung flow end-to-end (averaging bug confirmed fixed)
- Cleaned GitHub repo: deleted history with exposed password (`Immer_Strammes_Seill!`), recreated clean
- Updated .gitignore to exclude instance/, backups/, doc_archives/, *.db, secret_key, .env, CLAUDE.md
- Verified all 39 raw scores + normalizations for Event 2 (NRW Cup 2026 test data, 13 teams, 3 rounds)
- Verified Kürfiguren consistency and mutual exclusion across all judges
- Flagged round-dropping rule conflict (code=lowest, Excel=highest) — UNRESOLVED
- Corrected Kürfiguren rule: ONE per group (not one total)
- Changed "Wertebereich" column to "Max. Wert" on figures page
- Standardized button sizes across ~20 templates (removed btn-sm, unified CSS)
- Matched badge sizing to button sizing with !important overrides
- Synced full codebase from dev (30) to Pi (83) via rsync
- Changed app_main.py db_path from hardcoded to auto-detect
- Restarted Flask on both dev (30) and Pi (83) — both running, all 12 blueprints registered
- All committed to Gitea, GitHub pushed when asked
